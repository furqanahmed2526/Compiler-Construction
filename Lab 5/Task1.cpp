...
type String e.g. String var = ' Hello ' ; 
 Regex reg1 = new
Regex(@"^(int|float|double)\s([A-Za-z|_][A-Za-z|0-9]{0,10})\s(=)\s([0-
9]+([.][0-9]+)?([e][+|-]?[0-9]+)?)\s(;)$"); // line of type int alpha = 2 

;
 Match category1 = reg1.Match(line);
Regex reg2 = new
Regex(@"^(String|char)\s([A-Za-z|_][A-Za-z|0-9]{0,10})\s(=)\s[']\s([A-Zaz|_][A-Za-z|0-9]{0,30})\s[']\s(;)$"); // line of type String alpha = ' 
Hello ' ;
...
// if a current lexeme is not a variable but 
a keyword then make a token such as: <keyword, int>
 else
{
